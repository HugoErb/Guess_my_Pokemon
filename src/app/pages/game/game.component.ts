      ...
      <header class="bg-slate-800 border-b border-slate-700 px-6 py-3 flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
          @if (room()?.status === 'playing') {
            @if (isMyTurn()) {
              <span class="flex items-center gap-1.5 px-3 py-1 bg-red-600/30 border border-red-500 rounded-full text-sm font-semibold text-red-400 animate-pulse">
                <span class="w-2 h-2 rounded-full bg-red-500 inline-block"></span>
                C'est ton tour !
              </span>
            } @else {
              <span class="flex items-center gap-1.5 px-3 py-1 bg-slate-700 border border-slate-600 rounded-full text-sm text-slate-400">
